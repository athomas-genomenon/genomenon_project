...
#csv imports
evi <- read.csv("evidence-7293-2026-01-08.csv", header = TRUE)
qa <- read.csv("qa-7293-2026-01-08.csv", header = TRUE)
...
target_tx <- "NM_001791.4"

# --- Build a QA lookup that picks the canonical RefSeq cDNA (pipe-delimited) ---
qa_cdna_best <- qa %>%
  select(gene, variant_desc, variant_type, cdna_variant) %>%
...
target_tx <- "NM_001791.4" # This is the canonical refseq transcript we care about for our test gene (CDC42).
target_tx_novers <- sub("\\.[0-9]+$", "", target_tx) # just in case the .4 version isnt in the ucsc table, this will at least grab the correct id (NM_001791)
...
uniprot <- jsonlite::fromJSON("cdc42_domains.json")
...
weighted_line <- if_else(protein_df$any_weighted_proband, linetype = "dashed", linetype = "solid")
...
p_protein <- ggplot() +
  ...
  # --- UniProt feature track (below protein baseline) ---
  geom_rect(
    data = domains_df,
  ...
  geom_text(
    data = domains_df,
    aes(x = (aa_min + aa_max) / 2, y = -0.24, label = label),   # <- FIXED
    inherit.aes = FALSE,
    size = 2.8, vjust = 1
  ) +
  
  # --- CDS exon track (coding-only, projected to AA) ---
  geom_rect(
    data = cds_df,
  ...
  geom_segment(
    ...
    aes(x = x_aa, xend = x_aa, y = y_base, yend = y_plot_aa, color = classification, linetype = !any_weighted_proband),
    linewidth = 0.3, alpha = 0.7 #making a dashed line here - linetype = "dashed"
  ) +
  # --- heads ---
  geom_point(
    data = variant_plot_protein_plot,
    aes(x = x_aa, y = y_plot_aa, color = classification, shape = variant_type, fill = ps3_applied),
    size = 2.8, stroke = 0.9, alpha = 0.95
  ) +
  # Optional: label the noncoding variants
  geom_text(
    data = noncoding_df,
  ...
  scale_shape_manual(values = vt_shape_map) +
  scale_fill_manual(values = c(`TRUE` = "black", `FALSE` = "white"), name = "PS3 applied") +
  guides(fill = guide_legend(override.aes = list(shape = 21, color = "black", stroke = 0.9, size = 3))) +
  scale_y_continuous(NULL, breaks = NULL) +
  labs(
    x = "Protein position (AA)",
    title = paste0(unique(variant_plot_df2$gene_symbol), " variants (", tx_name, ")"),
    subtitle = "Protein lane: AA-mapped variants. Lower lane: non-coding variants. Color=classification; Shape=variant type; Fill=PS3 applied"
  ) +
  theme_classic()
